...
HighSpeedEncoderRosI::HighSpeedEncoderRosI(ros::NodeHandle nh, ros::NodeHandle nh_private) : nh_(nh), nh_private_(nh_private)
{
  ...
  nh_private_.param("SPEED_FILTER_SAMPLES_LEN", speed_filter_samples_len_, 10);
  nh_private_.param("SPEED_FILTER_IDLE_ITER_LOOPS_BEFORE_RESET", speed_filter_idle_iter_loops_before_reset_, 1);
  ...
  for (unsigned int i = 0; i < N; i++)
  {
    ...
    if (speed_filter_samples_len_ > 0)
    {
      ...
      else
      {
        ...
        if (spc.speeds_buffer.size() >= static_cast<size_t>(speed_filter_samples_len_))
        {
...
void HighSpeedEncoderRosI::positionChangeHandler(int index, int time, int positionChange)
{
