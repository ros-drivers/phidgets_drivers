...
int Imu::SpatialDataHandler(CPhidgetSpatialHandle /* handle */, void *userptr, CPhidgetSpatial_SpatialEventDataHandle *data, int count)
{
  ...
  {
    double ts = data[i]->timestamp.seconds + (data[i]->timestamp.microseconds / 1000.0 / 1000.0);
    ((Imu*)userptr)->dataHandler(data[i]->acceleration, data[i]->angularRate, data[i]->magneticField, ts);
  }
  return 0;
}

void Imu::dataHandler(const double acceleration[3], const double angularRate[3], const double magneticField[3], double timestamp)
{
...
int Imu::setCompassCorrectionParameters(double cc_mag_field, double cc_offset0, double cc_offset1, double cc_offset2,
...
{
  return CPhidgetSpatial_setCompassCorrectionParameters(imu_handle_, cc_mag_field,
                                                        cc_offset0, cc_offset1, cc_offset2, cc_gain0, cc_gain1, cc_gain2,
                                                        cc_T0, cc_T1, cc_T2, cc_T3, cc_T4, cc_T5);
}
